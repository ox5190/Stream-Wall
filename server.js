...
async function resolveYouTubeLive(raw) {
  ...
  if (!target) return { ok: false, error: '유튜브 영상 ID, 채널 ID, 핸들, 맞춤 URL을 인식하지 못했습니다.' };
  ...
  if (!response.ok) return { ok: false, error: '유튜브 라이브 페이지를 불러오지 못했습니다.' };
  ...
  return { ok: false, error: '현재 진행 중인 유튜브 라이브 영상을 찾지 못했습니다.' };
}
...
async function handleApi(req, res, url) {
  ...
  if (!target.trim()) {
    sendJson(res, 400, { ok: false, error: 'target 값이 필요합니다.' });
    return true;
  }
  ...
  try {
    ...
    sendJson(res, 502, { ok: false, error: '유튜브 라이브 확인 중 오류가 발생했습니다.' });
  }
  return true;
}
